...
export function CDashboard() {
  ...
  useEffect(() => {
    const fetchData = async () => {
      ...
      if (profile && tmpUser) {
        ...
        Axios.get(
          "http://localhost:4000/getDetails?id=" + tmpUser.id + "&role=Donar"
        ).then((result) => {
          cnt++;
          console.log(result);
          isdone = false;
  ...
  function seek() {
    ...
    Axios.get("http://localhost:4000/getDetails?role=Seeker").then((result) => {
      cnt++;
      console.log(result);
  ...
  function donate() {
    var cnt = 0;
    Axios.get("http://localhost:4000/getDetails?role=Donar").then((result) => {
      cnt++;
      console.log(result);
    ...
    return (
      <div className="" id="home">
              ...
              <div className="bg-white tm-block h-100">
                <div className="row">
                  ...
                  <div className="col-md-8 col-sm-12">
                    <Button
                      variant="info"
                      className="btn btn-small btn-primary space"
                      onClick={() => donate()}
                    >
                      Donar
                    </Button>
        ...
        <Modal
          ...
          metamaskid={data[index] ? data[index].metamaskid : ""}
        >
          <Table className="table">
            <tbody>
              <tr>
                <th>Hospital Name </th>
                <td> {data[index] ? data[index].hosname : ""}</td>
              </tr>
              <tr>
                <th>Email Id </th>
                <td> {data[index] ? data[index].email : ""}</td>
              </tr>
              <tr>
                <th>Mobile No </th>
                <td> {data[index] ? data[index].mobileno : ""}</td>
              </tr>
              <tr>
                <th>Address </th>
                <td> {data[index] ? data[index].address : ""}</td>
              </tr>
              <tr>
                <th>Country </th>
                <td> {data[index] ? data[index].country : ""}</td>
              </tr>
              <tr>
                <th>State </th>
                <td> {data[index] ? data[index].state : ""}</td>
              </tr>
              <tr>
                <th>District </th>
                <td> {data[index] ? data[index].district : ""}</td>
              </tr>
              <tr>
                <th>City </th>
                <td> {data[index] ? data[index].city : ""}</td>
              </tr>
            </tbody>
          </Table>
        </Modal>
