  ...
  try {
    ...
    if (collection === "users") {
      UserRegistration.find(
        { email: req.body.email, role: collection },
        function (err, docs) {
          if (err) throw err;
          console.log({ docs });
          if (docs.length > 0) {
            ...
            if (!passwordIsValid) {
              ...
              var token = jwt.sign({ id: docs[0].id }, "makemebelieveit", {
            ...
            res
              .status(203)
              .json({ message: "Invalid Email Id or Invalid Role" });
          }
          // console.log("Hello");
        }
      );
    } else if (collection === "Hospitals") {
      HospitalRegistration.find(
        { email: req.body.email, role: collection },
        function (err, docs) {
          if (err) throw err;
          if (docs.length > 0) {
            ...
            res
              .status(203)
              .send({ message: " Invalid Email Id or Invalid Role", err: err });
          }
          // console.log("Hello");
        }
      );
        ...
        UserRegistration.find(
          ...
          function (err, docs) {
            ...
            if (docs.length > 0) {
              ...
              console.log(docs);
              res.status(203).send({
                message: "Invalid Email Id or Invalid Role",
                err: err,
              });
            }
            // console.log("Hello");
